...
class Products extends Component {
    ...
    // const Test = prod.map(product => {
    //     return <Product key={product.nome} legend={product.nome} sublegend={"Preço: R$" + product.preco} src={product.url} amount="10"/>
    // });

    // const Test = (JSON.parse(this.state.products) || []).map(function(item, i) {
    //     return <AlertMsg key={i} color="error" msg="dasdas"/>
    //     // return <Product key={i} legend={item.nome} sublegend={"Preço: R$" + item.preco} src={item.url} amount="10"/>
    //   });
        const Test = null;
	return(
        <div>
